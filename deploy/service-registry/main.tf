...
resource "aws_lambda_function" "service-registry_lambda" {

  filename         = "./../service-registry/build-test1/rest-api-lambda-package.zip"
  function_name    = "${var.restApiLambdaFunctionName}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          =  "${var.restApiLambdaFunctionName}.handler"
  source_code_hash = "${base64sha256(file("./../service-registry/build-test1/rest-api-lambda-package.zip"))}"
  runtime          = "nodejs4.3"
  timeout          = "60"
  memory_size      = "1024"
}
...
resource "aws_api_gateway_integration" "service_registry_api_method-integration" {
  ...
  type = "AWS_PROXY"
  uri = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.region}:${var.account_id}:function:${aws_lambda_function.service-registry_lambda.function_name}/invocations"
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  function_name = "${aws_lambda_function.service-registry_lambda.arn}"
