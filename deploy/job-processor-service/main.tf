...
resource "aws_lambda_function" "job-processor-service_lambda" {
  filename         = "./../job-processor-service/build-test1/rest-api-lambda-package.zip"
  function_name    = "${var.restApiLambdaFunctionName}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "${var.restApiLambdaModuleName}.handler"
  source_code_hash = "${base64sha256(file("./../job-processor-service/build-test1/rest-api-lambda-package.zip"))}"
  runtime          = "nodejs4.3"
  timeout          = "60"
  memory_size      = "1024"
}
