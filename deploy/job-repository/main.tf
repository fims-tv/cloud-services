...
#################################
#  Lambda : rest-api-job-repository_lambda
#################################
...
resource "aws_api_gateway_integration" "job_repository_api_method-integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.region}:${var.account_id}:function:${aws_lambda_function.job-repository_lambda.function_name}/invocations"
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  function_name = "${aws_lambda_function.job-repository_lambda.arn}"
  principal     = "apigateway.amazonaws.com"

  # More: http://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-control-access-using-iam-policies-to-invoke-api.html
  source_arn = "arn:aws:execute-api:${var.region}:${var.account_id}:${aws_api_gateway_rest_api.job_repository_api.id}/*/${aws_api_gateway_method.job_repository_api_method.http_method}/*"
}
...
output "rest_service_url" {
  value = "https://${aws_api_gateway_deployment.job_repository_deployment.rest_api_id}.execute-api.${var.region}.amazonaws.com/${aws_api_gateway_deployment.job_repository_deployment.stage_name}"
}

output "lambda_arn" {
  value = "${aws_lambda_function.job-repository_lambda.arn}"
}
