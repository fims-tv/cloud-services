...
var repository = require("./fims-media-repository.js");
...
exports.handler = (event, context, callback) => {
    ...
    return processResource(event, event.body, function (err, resource) {
        console.log("processResource = resource =>", resource  );
        console.log("processResource = event.body =>", event.body  );

        if (err) {
            console.error("processResource error =>", err  );
            return done(400);
        } else {
            if (resource) {
        ...
        repository.getAll(event.stageVariables.TableName, resourceDescriptor.type, function (err, data) {
            if (err) {
                ...
                return done(500);
                 ...
                 console.log("RAW data from GetAll",  data);
...
function processResource(event, input, callback) {
    if (input) {
        ...
        console.log("resource before : jsonld.compact ==>",resource);
        ...
        console.log("resource after : jsonld.compact ==>",resource);
...
function renderResource(event, resource, callback) {
     console.log("renderResource = event =>", event);
     console.log("renderResource = resource =>", resource);
