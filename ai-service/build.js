...
var WORKER_RESULT_LAMBDA_PACKAGE_FILE = "./build/worker-result-lambda-package.zip";
...
function createWorkerResultLambdaPackage(callback) {
    if (!fs.existsSync("./build")) {
        fs.mkdirSync("./build");
    }

    var output = fs.createWriteStream(WORKER_RESULT_LAMBDA_PACKAGE_FILE);
    var archive = archiver("zip", { zlib: { level: 9 } });

    output.on("close", function () {
        console.log("Created '" + WORKER_RESULT_LAMBDA_PACKAGE_FILE + "' with size of " + archive.pointer() + " bytes");
        callback();
    });
    ...
    archive.file("lambda-worker-result.js");
    archive.directory("node_modules/async/");
    archive.directory("node_modules/fims-aws/");
    archive.directory("node_modules/request/");
    archive.directory("node_modules/uuid/");
...
function deployLambda(callback) {
    ...
    async.waterfall([
        ...
        createWorkerResultLambdaPackage,
...
switch (command) {
    case "all":
        ...
        functions.push(createWorkerResultLambdaPackage);
