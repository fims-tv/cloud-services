...
function parseUrl(event, url, callback) {
    if (url && url.startsWith(event.stageVariables.PublicUrl)) {
        var parts = url.substring(event.stageVariables.PublicUrl.length + 1).split("/");
    ...
    getDocClient().query(params, function (err, data) {
        ...
        if (!err) {
            ...
            data.Items.forEach(i => {
                items.push(replace(i.resource, INTERNAL, event.stageVariables.PublicUrl));
            });
        }
...
function getSingle(event, type, id, callback) {
    ...
    getDocClient().get(params, function (err, data) {
        ...
        if (!resource) {
            ...
            resource = replace(resource, INTERNAL, event.stageVariables.PublicUrl);
        }
...
function put(event, resource, callback) {
    parseUrl(event, resource.id, (err, type, id) => {
        ...
        var item = {
            "resource_type": type,
            "resource_id": id,
            "resource": replace(resource, event.stageVariables.PublicUrl, INTERNAL)
        };
